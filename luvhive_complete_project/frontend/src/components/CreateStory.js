...
const CreateStory = ({ user, onClose, onStoryCreated }) => {
  ...
  const handleMediaUpload = (event) => {
    ...
    if (isImage && file.size > maxImageSize) {
      alert(`📷 Story इमेज बहुत बड़ी है (${fileSizeMB}MB)।\n\n✅ समाधान:\n• अधिकतम साइज: 20MB\n• सुझाव: कंप्रेशन का उपयोग करें`);
      return;
    }
    
    if (isVideo && file.size > maxVideoSize) {
      alert(`🎥 Story वीडियो बहुत बड़ा है (${fileSizeMB}MB)।\n\n✅ समाधान:\n• अधिकतम साइज: 50MB\n• सुझाव: वीडियो को छोटा करें या कंप्रेस करें`);
      return;
    }
    ...
    if ((selectedImage && selectedImageFile) || (selectedVideo && selectedVideoFile)) {
      try {
        ...
        if (errorMsg.includes('केवल इमेज फाइलें')) {
          ...
          alert(`⚠️ Story अपलोड असफल: ${errorMsg}\n\nकृपया फिर से प्रयास करें।`);
        }
        return;
      }
    }
